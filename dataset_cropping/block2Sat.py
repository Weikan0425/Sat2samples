...
with rasterio.open(merged_image_path, 'w', driver='GTiff', width=image_width, height=image_height, count=1, dtype=np.uint8, crs=None, transform=None) as dst:
    # 初始化一个数组来保存合并后的图像数据
    merged_image_data = np.zeros((1, image_height, image_width), dtype=np.uint8)
